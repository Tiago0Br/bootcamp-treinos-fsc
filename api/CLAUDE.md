...
API de treinos construida com Fastify 5, TypeScript, Prisma 7 e Better-Auth. Roda em Node.js 24.x com pnpm 10.30+ (ambos obrigatorios via `engine-strict`).
...
- **Use Cases** (`src/use-cases/`) — Classes de logica de negocio. Recebem DTOs, usam transacoes Prisma para atomicidade (ex: desativar planos ativos antes de criar novos). Uma classe por caso de uso.
...
PostgreSQL 16 via Docker. Prisma client inicializado em `src/lib/prisma.ts`. Tipos gerados em `src/generated/prisma/` (gitignored). Schema em `prisma/schema.prisma`.
...
- Variaveis de ambiente específicadas no arquivo `/src/env.ts`