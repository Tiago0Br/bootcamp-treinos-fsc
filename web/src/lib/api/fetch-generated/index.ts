/**
 * Generated by orval v8.5.3 🍺
 * Do not edit manually.
 * Bootcamp Treinos API
 * API para o bootcamp de treinos do FSC
 * OpenAPI spec version: 1.0.0
 */
import { customFetch } from '../../fetch'
export type GetHomeData200TodayWorkoutDayWeekDay =
  (typeof GetHomeData200TodayWorkoutDayWeekDay)[keyof typeof GetHomeData200TodayWorkoutDayWeekDay]

export const GetHomeData200TodayWorkoutDayWeekDay = {
  MONDAY: 'MONDAY',
  TUESDAY: 'TUESDAY',
  WEDNESDAY: 'WEDNESDAY',
  THURSDAY: 'THURSDAY',
  FRIDAY: 'FRIDAY',
  SATURDAY: 'SATURDAY',
  SUNDAY: 'SUNDAY'
} as const

export type GetHomeData200TodayWorkoutDay = {
  /** @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$ */
  workoutPlanId: string
  /** @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$ */
  id: string
  name: string
  isRest: boolean
  weekDay: GetHomeData200TodayWorkoutDayWeekDay
  estimatedDurationInSeconds: number
  coverImageUrl?: string
  exercisesCount: number
}

export type GetHomeData200ConsistencyByDay = {
  [key: string]: {
    workoutDayCompleted: boolean
    workoutDayStarted: boolean
  }
}

export type GetHomeData200 = {
  /** @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$ */
  activeWorkoutPlanId?: string
  todayWorkoutDay?: GetHomeData200TodayWorkoutDay
  workoutStreak: number
  consistencyByDay: GetHomeData200ConsistencyByDay
}

export type GetHomeData401 = {
  error: string
  code: string
}

export type GetHomeData500 = {
  error: string
  code: string
}

/**
 * @nullable
 */
export type GetUserTrainData200 = {
  userId: string
  userName: string
  weightInGrams: number
  heightInCentimeters: number
  age: number
  bodyFatPercentage: number
} | null

export type GetUserTrainData401 = {
  error: string
  code: string
}

export type GetUserTrainData500 = {
  error: string
  code: string
}

export type CreateOrUpdateUserTrainDataBody = {
  /** @minimum 1 */
  weightInGrams: number
  /** @minimum 1 */
  heightInCentimeters: number
  /** @minimum 1 */
  age: number
  /**
   * @minimum 0
   * @maximum 1
   */
  bodyFatPercentage: number
}

export type CreateOrUpdateUserTrainData200 = {
  userId: string
  userName: string
  weightInGrams: number
  heightInCentimeters: number
  age: number
  bodyFatPercentage: number
}

export type CreateOrUpdateUserTrainData401 = {
  error: string
  code: string
}

export type CreateOrUpdateUserTrainData500 = {
  error: string
  code: string
}

export type GetWorkoutStatsParams = {
  /**
   * @pattern ^(?:(?:\d\d[2468][048]|\d\d[13579][26]|\d\d0[48]|[02468][048]00|[13579][26]00)-02-29|\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\d|30)|(?:02)-(?:0[1-9]|1\d|2[0-8])))$
   */
  from: string
  /**
   * @pattern ^(?:(?:\d\d[2468][048]|\d\d[13579][26]|\d\d0[48]|[02468][048]00|[13579][26]00)-02-29|\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\d|30)|(?:02)-(?:0[1-9]|1\d|2[0-8])))$
   */
  to: string
}

export type GetWorkoutStats200ConsistencyByDay = {
  [key: string]: {
    workoutDayCompleted: boolean
    workoutDayStarted: boolean
  }
}

export type GetWorkoutStats200 = {
  workoutStreak: number
  consistencyByDay: GetWorkoutStats200ConsistencyByDay
  completedWorkoutsCount: number
  conclusionRate: number
  totalTimeInSeconds: number
}

export type GetWorkoutStats401 = {
  error: string
  code: string
}

export type GetWorkoutStats500 = {
  error: string
  code: string
}

export type ListWorkoutPlansParams = {
  active?: ListWorkoutPlansActive
}

export type ListWorkoutPlansActive =
  (typeof ListWorkoutPlansActive)[keyof typeof ListWorkoutPlansActive]

export const ListWorkoutPlansActive = {
  true: 'true',
  false: 'false'
} as const

export type ListWorkoutPlans200ItemWorkoutDaysItemWeekDay =
  (typeof ListWorkoutPlans200ItemWorkoutDaysItemWeekDay)[keyof typeof ListWorkoutPlans200ItemWorkoutDaysItemWeekDay]

export const ListWorkoutPlans200ItemWorkoutDaysItemWeekDay = {
  MONDAY: 'MONDAY',
  TUESDAY: 'TUESDAY',
  WEDNESDAY: 'WEDNESDAY',
  THURSDAY: 'THURSDAY',
  FRIDAY: 'FRIDAY',
  SATURDAY: 'SATURDAY',
  SUNDAY: 'SUNDAY'
} as const

export type ListWorkoutPlans200ItemWorkoutDaysItemExercisesItem = {
  /** @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$ */
  id: string
  order: number
  name: string
  sets: number
  reps: number
  restTimeInSeconds: number
}

export type ListWorkoutPlans200ItemWorkoutDaysItem = {
  /** @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$ */
  id: string
  name: string
  weekDay: ListWorkoutPlans200ItemWorkoutDaysItemWeekDay
  isRest: boolean
  estimatedDurationInSeconds: number
  coverImageUrl?: string
  exercises: ListWorkoutPlans200ItemWorkoutDaysItemExercisesItem[]
}

export type ListWorkoutPlans200Item = {
  /** @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$ */
  id: string
  name: string
  isActive: boolean
  workoutDays: ListWorkoutPlans200ItemWorkoutDaysItem[]
}

export type ListWorkoutPlans401 = {
  error: string
  code: string
}

export type ListWorkoutPlans500 = {
  error: string
  code: string
}

export type CreateWorkoutPlansBodyWorkoutDaysItemWeekDay =
  (typeof CreateWorkoutPlansBodyWorkoutDaysItemWeekDay)[keyof typeof CreateWorkoutPlansBodyWorkoutDaysItemWeekDay]

export const CreateWorkoutPlansBodyWorkoutDaysItemWeekDay = {
  MONDAY: 'MONDAY',
  TUESDAY: 'TUESDAY',
  WEDNESDAY: 'WEDNESDAY',
  THURSDAY: 'THURSDAY',
  FRIDAY: 'FRIDAY',
  SATURDAY: 'SATURDAY',
  SUNDAY: 'SUNDAY'
} as const

export type CreateWorkoutPlansBodyWorkoutDaysItemExercicesItem = {
  /** @minLength 1 */
  name: string
  /** @minimum 0 */
  order: number
  /** @minimum 1 */
  sets: number
  /** @minimum 1 */
  reps: number
  /** @minimum 1 */
  restTimeInSeconds: number
}

export type CreateWorkoutPlansBodyWorkoutDaysItem = {
  /** @minLength 1 */
  name: string
  weekDay: CreateWorkoutPlansBodyWorkoutDaysItemWeekDay
  isRest?: boolean
  /** @minimum 1 */
  estimatedDurationInSeconds: number
  coverImageUrl?: string
  exercices: CreateWorkoutPlansBodyWorkoutDaysItemExercicesItem[]
}

export type CreateWorkoutPlansBody = {
  /** @minLength 1 */
  name: string
  workoutDays: CreateWorkoutPlansBodyWorkoutDaysItem[]
}

export type CreateWorkoutPlans201 = {
  /** @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$ */
  workoutPlanId: string
}

export type CreateWorkoutPlans400 = {
  error: string
  code: string
}

export type CreateWorkoutPlans401 = {
  error: string
  code: string
}

export type CreateWorkoutPlans500 = {
  error: string
  code: string
}

export type GetWorkoutPlanById200WorkoutDaysItemWeekDay =
  (typeof GetWorkoutPlanById200WorkoutDaysItemWeekDay)[keyof typeof GetWorkoutPlanById200WorkoutDaysItemWeekDay]

export const GetWorkoutPlanById200WorkoutDaysItemWeekDay = {
  MONDAY: 'MONDAY',
  TUESDAY: 'TUESDAY',
  WEDNESDAY: 'WEDNESDAY',
  THURSDAY: 'THURSDAY',
  FRIDAY: 'FRIDAY',
  SATURDAY: 'SATURDAY',
  SUNDAY: 'SUNDAY'
} as const

export type GetWorkoutPlanById200WorkoutDaysItem = {
  /** @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$ */
  id: string
  weekDay: GetWorkoutPlanById200WorkoutDaysItemWeekDay
  name: string
  isRest: boolean
  coverImageUrl?: string
  estimatedDurationInSeconds: number
  exercisesCount: number
}

export type GetWorkoutPlanById200 = {
  /** @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$ */
  id: string
  name: string
  workoutDays: GetWorkoutPlanById200WorkoutDaysItem[]
}

export type GetWorkoutPlanById401 = {
  error: string
  code: string
}

export type GetWorkoutPlanById403 = {
  error: string
  code: string
}

export type GetWorkoutPlanById404 = {
  error: string
  code: string
}

export type GetWorkoutPlanById500 = {
  error: string
  code: string
}

export type GetWorkoutDayById200WeekDay =
  (typeof GetWorkoutDayById200WeekDay)[keyof typeof GetWorkoutDayById200WeekDay]

export const GetWorkoutDayById200WeekDay = {
  MONDAY: 'MONDAY',
  TUESDAY: 'TUESDAY',
  WEDNESDAY: 'WEDNESDAY',
  THURSDAY: 'THURSDAY',
  FRIDAY: 'FRIDAY',
  SATURDAY: 'SATURDAY',
  SUNDAY: 'SUNDAY'
} as const

export type GetWorkoutDayById200ExercisesItem = {
  /** @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$ */
  id: string
  name: string
  /** @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$ */
  workoutDayId: string
  order: number
  sets: number
  reps: number
  restTimeInSeconds: number
}

export type GetWorkoutDayById200SessionsItem = {
  /** @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$ */
  id: string
  /** @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$ */
  workoutDayId: string
  /** @pattern ^(?:(?:\d\d[2468][048]|\d\d[13579][26]|\d\d0[48]|[02468][048]00|[13579][26]00)-02-29|\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\d|30)|(?:02)-(?:0[1-9]|1\d|2[0-8])))T(?:(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d(?:\.\d+)?)?(?:Z))$ */
  startedAt?: string
  /** @pattern ^(?:(?:\d\d[2468][048]|\d\d[13579][26]|\d\d0[48]|[02468][048]00|[13579][26]00)-02-29|\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\d|30)|(?:02)-(?:0[1-9]|1\d|2[0-8])))T(?:(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d(?:\.\d+)?)?(?:Z))$ */
  completedAt?: string
}

export type GetWorkoutDayById200 = {
  /** @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$ */
  id: string
  name: string
  isRest: boolean
  coverImageUrl?: string
  estimatedDurationInSeconds: number
  weekDay: GetWorkoutDayById200WeekDay
  exercises: GetWorkoutDayById200ExercisesItem[]
  sessions: GetWorkoutDayById200SessionsItem[]
}

export type GetWorkoutDayById401 = {
  error: string
  code: string
}

export type GetWorkoutDayById403 = {
  error: string
  code: string
}

export type GetWorkoutDayById404 = {
  error: string
  code: string
}

export type GetWorkoutDayById500 = {
  error: string
  code: string
}

export type StartWorkoutSession201 = {
  /** @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$ */
  userWorkoutSessionId: string
}

export type StartWorkoutSession401 = {
  error: string
  code: string
}

export type StartWorkoutSession403 = {
  error: string
  code: string
}

export type StartWorkoutSession404 = {
  error: string
  code: string
}

export type StartWorkoutSession409 = {
  error: string
  code: string
}

export type StartWorkoutSession422 = {
  error: string
  code: string
}

export type StartWorkoutSession500 = {
  error: string
  code: string
}

export type UpdateWorkoutSessionBody = {
  /** @pattern ^(?:(?:\d\d[2468][048]|\d\d[13579][26]|\d\d0[48]|[02468][048]00|[13579][26]00)-02-29|\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\d|30)|(?:02)-(?:0[1-9]|1\d|2[0-8])))T(?:(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d(?:\.\d+)?)?(?:Z))$ */
  completedAt: string
}

export type UpdateWorkoutSession200 = {
  /** @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$ */
  id: string
  /** @pattern ^(?:(?:\d\d[2468][048]|\d\d[13579][26]|\d\d0[48]|[02468][048]00|[13579][26]00)-02-29|\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\d|30)|(?:02)-(?:0[1-9]|1\d|2[0-8])))T(?:(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d(?:\.\d+)?)?(?:Z))$ */
  startedAt: string
  /** @pattern ^(?:(?:\d\d[2468][048]|\d\d[13579][26]|\d\d0[48]|[02468][048]00|[13579][26]00)-02-29|\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\d|30)|(?:02)-(?:0[1-9]|1\d|2[0-8])))T(?:(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d(?:\.\d+)?)?(?:Z))$ */
  completedAt: string
}

export type UpdateWorkoutSession401 = {
  error: string
  code: string
}

export type UpdateWorkoutSession403 = {
  error: string
  code: string
}

export type UpdateWorkoutSession404 = {
  error: string
  code: string
}

export type UpdateWorkoutSession500 = {
  error: string
  code: string
}

export type Get200 = {
  message: string
}

/**
 * @summary Get home page data for a specific date
 */
export type getHomeDataResponse200 = {
  data: GetHomeData200
  status: 200
}

export type getHomeDataResponse401 = {
  data: GetHomeData401
  status: 401
}

export type getHomeDataResponse500 = {
  data: GetHomeData500
  status: 500
}

export type getHomeDataResponseSuccess = getHomeDataResponse200 & {
  headers: Headers
}
export type getHomeDataResponseError = (
  | getHomeDataResponse401
  | getHomeDataResponse500
) & {
  headers: Headers
}

export type getHomeDataResponse =
  | getHomeDataResponseSuccess
  | getHomeDataResponseError

export const getGetHomeDataUrl = (date: string) => {
  return `/home/${date}`
}

export const getHomeData = async (
  date: string,
  options?: RequestInit
): Promise<getHomeDataResponse> => {
  return customFetch<getHomeDataResponse>(getGetHomeDataUrl(date), {
    ...options,
    method: 'GET'
  })
}

/**
 * @summary Get current user train data
 */
export type getUserTrainDataResponse200 = {
  data: GetUserTrainData200
  status: 200
}

export type getUserTrainDataResponse401 = {
  data: GetUserTrainData401
  status: 401
}

export type getUserTrainDataResponse500 = {
  data: GetUserTrainData500
  status: 500
}

export type getUserTrainDataResponseSuccess = getUserTrainDataResponse200 & {
  headers: Headers
}
export type getUserTrainDataResponseError = (
  | getUserTrainDataResponse401
  | getUserTrainDataResponse500
) & {
  headers: Headers
}

export type getUserTrainDataResponse =
  | getUserTrainDataResponseSuccess
  | getUserTrainDataResponseError

export const getGetUserTrainDataUrl = () => {
  return `/me`
}

export const getUserTrainData = async (
  options?: RequestInit
): Promise<getUserTrainDataResponse> => {
  return customFetch<getUserTrainDataResponse>(getGetUserTrainDataUrl(), {
    ...options,
    method: 'GET'
  })
}

/**
 * @summary Create or update current user train data
 */
export type createOrUpdateUserTrainDataResponse200 = {
  data: CreateOrUpdateUserTrainData200
  status: 200
}

export type createOrUpdateUserTrainDataResponse401 = {
  data: CreateOrUpdateUserTrainData401
  status: 401
}

export type createOrUpdateUserTrainDataResponse500 = {
  data: CreateOrUpdateUserTrainData500
  status: 500
}

export type createOrUpdateUserTrainDataResponseSuccess =
  createOrUpdateUserTrainDataResponse200 & {
    headers: Headers
  }
export type createOrUpdateUserTrainDataResponseError = (
  | createOrUpdateUserTrainDataResponse401
  | createOrUpdateUserTrainDataResponse500
) & {
  headers: Headers
}

export type createOrUpdateUserTrainDataResponse =
  | createOrUpdateUserTrainDataResponseSuccess
  | createOrUpdateUserTrainDataResponseError

export const getCreateOrUpdateUserTrainDataUrl = () => {
  return `/me/me`
}

export const createOrUpdateUserTrainData = async (
  createOrUpdateUserTrainDataBody: CreateOrUpdateUserTrainDataBody,
  options?: RequestInit
): Promise<createOrUpdateUserTrainDataResponse> => {
  return customFetch<createOrUpdateUserTrainDataResponse>(
    getCreateOrUpdateUserTrainDataUrl(),
    {
      ...options,
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', ...options?.headers },
      body: JSON.stringify(createOrUpdateUserTrainDataBody)
    }
  )
}

/**
 * @summary Get workout stats for a date range
 */
export type getWorkoutStatsResponse200 = {
  data: GetWorkoutStats200
  status: 200
}

export type getWorkoutStatsResponse401 = {
  data: GetWorkoutStats401
  status: 401
}

export type getWorkoutStatsResponse500 = {
  data: GetWorkoutStats500
  status: 500
}

export type getWorkoutStatsResponseSuccess = getWorkoutStatsResponse200 & {
  headers: Headers
}
export type getWorkoutStatsResponseError = (
  | getWorkoutStatsResponse401
  | getWorkoutStatsResponse500
) & {
  headers: Headers
}

export type getWorkoutStatsResponse =
  | getWorkoutStatsResponseSuccess
  | getWorkoutStatsResponseError

export const getGetWorkoutStatsUrl = (params: GetWorkoutStatsParams) => {
  const normalizedParams = new URLSearchParams()

  Object.entries(params || {}).forEach(([key, value]) => {
    if (value !== undefined) {
      normalizedParams.append(key, value === null ? 'null' : value.toString())
    }
  })

  const stringifiedParams = normalizedParams.toString()

  return stringifiedParams.length > 0 ? `/stats?${stringifiedParams}` : `/stats`
}

export const getWorkoutStats = async (
  params: GetWorkoutStatsParams,
  options?: RequestInit
): Promise<getWorkoutStatsResponse> => {
  return customFetch<getWorkoutStatsResponse>(getGetWorkoutStatsUrl(params), {
    ...options,
    method: 'GET'
  })
}

/**
 * @summary List workout plans
 */
export type listWorkoutPlansResponse200 = {
  data: ListWorkoutPlans200Item[]
  status: 200
}

export type listWorkoutPlansResponse401 = {
  data: ListWorkoutPlans401
  status: 401
}

export type listWorkoutPlansResponse500 = {
  data: ListWorkoutPlans500
  status: 500
}

export type listWorkoutPlansResponseSuccess = listWorkoutPlansResponse200 & {
  headers: Headers
}
export type listWorkoutPlansResponseError = (
  | listWorkoutPlansResponse401
  | listWorkoutPlansResponse500
) & {
  headers: Headers
}

export type listWorkoutPlansResponse =
  | listWorkoutPlansResponseSuccess
  | listWorkoutPlansResponseError

export const getListWorkoutPlansUrl = (params?: ListWorkoutPlansParams) => {
  const normalizedParams = new URLSearchParams()

  Object.entries(params || {}).forEach(([key, value]) => {
    if (value !== undefined) {
      normalizedParams.append(key, value === null ? 'null' : value.toString())
    }
  })

  const stringifiedParams = normalizedParams.toString()

  return stringifiedParams.length > 0
    ? `/workout-plans/?${stringifiedParams}`
    : `/workout-plans/`
}

export const listWorkoutPlans = async (
  params?: ListWorkoutPlansParams,
  options?: RequestInit
): Promise<listWorkoutPlansResponse> => {
  return customFetch<listWorkoutPlansResponse>(getListWorkoutPlansUrl(params), {
    ...options,
    method: 'GET'
  })
}

/**
 * @summary Create a new workout plan
 */
export type createWorkoutPlansResponse201 = {
  data: CreateWorkoutPlans201
  status: 201
}

export type createWorkoutPlansResponse400 = {
  data: CreateWorkoutPlans400
  status: 400
}

export type createWorkoutPlansResponse401 = {
  data: CreateWorkoutPlans401
  status: 401
}

export type createWorkoutPlansResponse500 = {
  data: CreateWorkoutPlans500
  status: 500
}

export type createWorkoutPlansResponseSuccess =
  createWorkoutPlansResponse201 & {
    headers: Headers
  }
export type createWorkoutPlansResponseError = (
  | createWorkoutPlansResponse400
  | createWorkoutPlansResponse401
  | createWorkoutPlansResponse500
) & {
  headers: Headers
}

export type createWorkoutPlansResponse =
  | createWorkoutPlansResponseSuccess
  | createWorkoutPlansResponseError

export const getCreateWorkoutPlansUrl = () => {
  return `/workout-plans`
}

export const createWorkoutPlans = async (
  createWorkoutPlansBody: CreateWorkoutPlansBody,
  options?: RequestInit
): Promise<createWorkoutPlansResponse> => {
  return customFetch<createWorkoutPlansResponse>(getCreateWorkoutPlansUrl(), {
    ...options,
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...options?.headers },
    body: JSON.stringify(createWorkoutPlansBody)
  })
}

/**
 * @summary Get a workout plan by id
 */
export type getWorkoutPlanByIdResponse200 = {
  data: GetWorkoutPlanById200
  status: 200
}

export type getWorkoutPlanByIdResponse401 = {
  data: GetWorkoutPlanById401
  status: 401
}

export type getWorkoutPlanByIdResponse403 = {
  data: GetWorkoutPlanById403
  status: 403
}

export type getWorkoutPlanByIdResponse404 = {
  data: GetWorkoutPlanById404
  status: 404
}

export type getWorkoutPlanByIdResponse500 = {
  data: GetWorkoutPlanById500
  status: 500
}

export type getWorkoutPlanByIdResponseSuccess =
  getWorkoutPlanByIdResponse200 & {
    headers: Headers
  }
export type getWorkoutPlanByIdResponseError = (
  | getWorkoutPlanByIdResponse401
  | getWorkoutPlanByIdResponse403
  | getWorkoutPlanByIdResponse404
  | getWorkoutPlanByIdResponse500
) & {
  headers: Headers
}

export type getWorkoutPlanByIdResponse =
  | getWorkoutPlanByIdResponseSuccess
  | getWorkoutPlanByIdResponseError

export const getGetWorkoutPlanByIdUrl = (id: string) => {
  return `/workout-plans/${id}`
}

export const getWorkoutPlanById = async (
  id: string,
  options?: RequestInit
): Promise<getWorkoutPlanByIdResponse> => {
  return customFetch<getWorkoutPlanByIdResponse>(getGetWorkoutPlanByIdUrl(id), {
    ...options,
    method: 'GET'
  })
}

/**
 * @summary Get a workout day by id
 */
export type getWorkoutDayByIdResponse200 = {
  data: GetWorkoutDayById200
  status: 200
}

export type getWorkoutDayByIdResponse401 = {
  data: GetWorkoutDayById401
  status: 401
}

export type getWorkoutDayByIdResponse403 = {
  data: GetWorkoutDayById403
  status: 403
}

export type getWorkoutDayByIdResponse404 = {
  data: GetWorkoutDayById404
  status: 404
}

export type getWorkoutDayByIdResponse500 = {
  data: GetWorkoutDayById500
  status: 500
}

export type getWorkoutDayByIdResponseSuccess = getWorkoutDayByIdResponse200 & {
  headers: Headers
}
export type getWorkoutDayByIdResponseError = (
  | getWorkoutDayByIdResponse401
  | getWorkoutDayByIdResponse403
  | getWorkoutDayByIdResponse404
  | getWorkoutDayByIdResponse500
) & {
  headers: Headers
}

export type getWorkoutDayByIdResponse =
  | getWorkoutDayByIdResponseSuccess
  | getWorkoutDayByIdResponseError

export const getGetWorkoutDayByIdUrl = (
  workoutPlanId: string,
  workoutDayId: string
) => {
  return `/workout-plans/${workoutPlanId}/days/${workoutDayId}`
}

export const getWorkoutDayById = async (
  workoutPlanId: string,
  workoutDayId: string,
  options?: RequestInit
): Promise<getWorkoutDayByIdResponse> => {
  return customFetch<getWorkoutDayByIdResponse>(
    getGetWorkoutDayByIdUrl(workoutPlanId, workoutDayId),
    {
      ...options,
      method: 'GET'
    }
  )
}

/**
 * @summary Start a workout session for a workout day
 */
export type startWorkoutSessionResponse201 = {
  data: StartWorkoutSession201
  status: 201
}

export type startWorkoutSessionResponse401 = {
  data: StartWorkoutSession401
  status: 401
}

export type startWorkoutSessionResponse403 = {
  data: StartWorkoutSession403
  status: 403
}

export type startWorkoutSessionResponse404 = {
  data: StartWorkoutSession404
  status: 404
}

export type startWorkoutSessionResponse409 = {
  data: StartWorkoutSession409
  status: 409
}

export type startWorkoutSessionResponse422 = {
  data: StartWorkoutSession422
  status: 422
}

export type startWorkoutSessionResponse500 = {
  data: StartWorkoutSession500
  status: 500
}

export type startWorkoutSessionResponseSuccess =
  startWorkoutSessionResponse201 & {
    headers: Headers
  }
export type startWorkoutSessionResponseError = (
  | startWorkoutSessionResponse401
  | startWorkoutSessionResponse403
  | startWorkoutSessionResponse404
  | startWorkoutSessionResponse409
  | startWorkoutSessionResponse422
  | startWorkoutSessionResponse500
) & {
  headers: Headers
}

export type startWorkoutSessionResponse =
  | startWorkoutSessionResponseSuccess
  | startWorkoutSessionResponseError

export const getStartWorkoutSessionUrl = (
  workoutPlanId: string,
  workoutDayId: string
) => {
  return `/workout-plans/${workoutPlanId}/days/${workoutDayId}/sessions`
}

export const startWorkoutSession = async (
  workoutPlanId: string,
  workoutDayId: string,
  options?: RequestInit
): Promise<startWorkoutSessionResponse> => {
  return customFetch<startWorkoutSessionResponse>(
    getStartWorkoutSessionUrl(workoutPlanId, workoutDayId),
    {
      ...options,
      method: 'POST'
    }
  )
}

/**
 * @summary Update a workout session
 */
export type updateWorkoutSessionResponse200 = {
  data: UpdateWorkoutSession200
  status: 200
}

export type updateWorkoutSessionResponse401 = {
  data: UpdateWorkoutSession401
  status: 401
}

export type updateWorkoutSessionResponse403 = {
  data: UpdateWorkoutSession403
  status: 403
}

export type updateWorkoutSessionResponse404 = {
  data: UpdateWorkoutSession404
  status: 404
}

export type updateWorkoutSessionResponse500 = {
  data: UpdateWorkoutSession500
  status: 500
}

export type updateWorkoutSessionResponseSuccess =
  updateWorkoutSessionResponse200 & {
    headers: Headers
  }
export type updateWorkoutSessionResponseError = (
  | updateWorkoutSessionResponse401
  | updateWorkoutSessionResponse403
  | updateWorkoutSessionResponse404
  | updateWorkoutSessionResponse500
) & {
  headers: Headers
}

export type updateWorkoutSessionResponse =
  | updateWorkoutSessionResponseSuccess
  | updateWorkoutSessionResponseError

export const getUpdateWorkoutSessionUrl = (
  workoutPlanId: string,
  workoutDayId: string,
  workoutSessionId: string
) => {
  return `/workout-plans/${workoutPlanId}/days/${workoutDayId}/sessions/${workoutSessionId}`
}

export const updateWorkoutSession = async (
  workoutPlanId: string,
  workoutDayId: string,
  workoutSessionId: string,
  updateWorkoutSessionBody: UpdateWorkoutSessionBody,
  options?: RequestInit
): Promise<updateWorkoutSessionResponse> => {
  return customFetch<updateWorkoutSessionResponse>(
    getUpdateWorkoutSessionUrl(workoutPlanId, workoutDayId, workoutSessionId),
    {
      ...options,
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json', ...options?.headers },
      body: JSON.stringify(updateWorkoutSessionBody)
    }
  )
}

/**
 * @summary Chat with AI personal trainer
 */
export type chatWithAIResponse200 = {
  data: void
  status: 200
}

export type chatWithAIResponseSuccess = chatWithAIResponse200 & {
  headers: Headers
}

export type chatWithAIResponse = chatWithAIResponseSuccess

export const getChatWithAIUrl = () => {
  return `/ai/`
}

export const chatWithAI = async (
  options?: RequestInit
): Promise<chatWithAIResponse> => {
  return customFetch<chatWithAIResponse>(getChatWithAIUrl(), {
    ...options,
    method: 'POST'
  })
}

/**
 * Verify if API is running
 * @summary Health Check
 */
export type getResponse200 = {
  data: Get200
  status: 200
}

export type getResponseSuccess = getResponse200 & {
  headers: Headers
}

export type getResponse = getResponseSuccess

export const getGetUrl = () => {
  return `/`
}

export const get = async (options?: RequestInit): Promise<getResponse> => {
  return customFetch<getResponse>(getGetUrl(), {
    ...options,
    method: 'GET'
  })
}
